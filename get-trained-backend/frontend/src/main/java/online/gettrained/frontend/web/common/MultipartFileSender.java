...
public class MultipartFileSender {
  ...
  public MultipartFileSender(BlobData data, HttpServletRequest request,
  ...
  public void serveResource() throws Exception {
    ...
    long lastModified = LocalDateTime.ofInstant(data.getDateCreate().toInstant(),
    ...
    if (ifNoneMatch != null && HttpUtils.matches(ifNoneMatch, data.getName())) {
      response.setHeader("ETag", data.getName()); // Required in 304.
    ...
    if (ifMatch != null && !HttpUtils.matches(ifMatch, data.getName())) {
    ...
    if (range != null) {
      ...
      if (ifRange != null && !ifRange.equals(data.getName())) {
        try {
    ...
    response.setHeader("Content-Disposition", disposition + ";filename=\"" + data.getName() + "\"");
    logger.debug("Content-Disposition : {}", disposition);
    response.setHeader("Accept-Ranges", "bytes");
    response.setHeader("ETag", data.getName());
    ...
    // Prepare streams.
    try (InputStream input = new BufferedInputStream(new ByteArrayInputStream(data.get()));
